...
__device__ __forceinline__ uint4 build_desc(const void* base) {
  uint4 desc;
  uintptr_t addr = reinterpret_cast<uintptr_t>(base);
  asm volatile(
      "s_mov_b32 %0, %2\n"
      "s_mov_b32 %1, 0xffffffff\n"
      "s_mov_b32 %2, 0\n"
      "s_mov_b32 %3, 0\n"
      : "=s"(desc.x), "=s"(desc.y), "=s"(desc.z), "=s"(desc.w)
      : "s"(addr)
  );
  return desc;
}
...
void fp8_mm_kernel(const __hip_fp8_e4m3_fnuz* A,
  ...
  size_t M_scale = (M + BLOCK_DIM - 1) / BLOCK_DIM;
  ...
  uint4 descA = build_desc(A);
  uint4 descB = build_desc(B);
  ...
  for (size_t tileOffset = 0; tileOffset < K; tileOffset += BK) {
      ...
      for (size_t innerRowOffsetA = 0; innerRowOffsetA < BK; innerRowOffsetA += strideA) {
          size_t row = innerRowOffsetA + innerRowA;
          size_t col = colOffsetA + innerColA * VECTOR_SIZE;
          if (row < K && (col + VECTOR_SIZE - 1) < N) {
              uint4 vA;
              asm volatile(
                  "raw_buffer_load_dwordx4 %0, %1, %2 offen offset:0\n"
                  : "=v"(vA)
                  : "v"(row * N + col), "s"(descA)
              );
      ...
      if (threadIdx.x < BN) {
          *reinterpret_cast<float4*>(&Ws[threadIdx.x * 4]) =
              *reinterpret_cast<const float4*>(
                  &A_scale[(tileOffset / BLOCK_DIM) * N + (colOffsetA + threadIdx.x * 4)]
              );
      }

      #pragma unroll
      for (size_t innerRowOffsetB = 0; innerRowOffsetB < BK; innerRowOffsetB += strideB) {
          size_t row = innerRowOffsetB + innerRowB;
          size_t col = colOffsetB + innerColB * VECTOR_SIZE;
          if (row < K && (col + VECTOR_SIZE - 1) < M) {
              uint4 vB;
              asm volatile(
                  "raw_buffer_load_dwordx4 %0, %1, %2 offen offset:0\n"
                  : "=v"(vB)
      ...
      if (threadIdx.x == numThreads - 1) {
          Ws[BN] = B_scale[(tileOffset / BLOCK_DIM) * M_scale + (colOffsetB / BLOCK_DIM)];
      }
        ...
        for (size_t BKOffset = 0; BKOffset < BK; BKOffset += 16) {
            for (size_t wn = 0; wn < WITERN; ++wn) {
                for (size_t i = 0; i < 8; ++i) {
                    a[wn][i] = As[BKOffset + warpY * 8 + i][warpRowOffset + wn * 32 + warpX];
                }
            }
            for (size_t wm = 0; wm < WITERM; ++wm) {
                for (size_t i = 0; i < 8; ++i) {
                    b[wm][i] = Bs[BKOffset + warpY * 8 + i][warpColOffset + wm * 32 + warpX];
                }
            }
            floatx16 c;
            for (size_t wn = 0; wn < WITERN; ++wn) {
                for (size_t wm = 0; wm < WITERM; ++wm) {
                    c = __builtin_amdgcn_mfma_f32_32x32x16_fp8_fp8(
                        ...
                        *reinterpret_cast<long*>(b[wm]),
                        c, 0, 0, 0
                    );
                    for (size_t j = 0; j < 4; ++j) for (size_t i = 0; i < 4; ++i)
                        d[wn][wm][i + j*4] +=
                          c[i + j*4] *
                          Ws[warpRowOffset + wn*32 + j*8 + warpY*4 + i] *
                          b_scale;
                }
            }
        }
        ...
        for (size_t wm = 0; wm < WITERM; ++wm) {
            for (size_t j = 0; j < 4; ++j) {
                for (size_t i = 0; i < 4; ++i) {
                    size_t r = rowOffsetC + warpRowOffset + wn*32 + j*8 + warpY*4 + i;
                    size_t c = colOffsetC + warpColOffset + wm*32 + warpX;
                    if (r < N && c < M) {
                        C[r*M + c] = (__hip_bfloat16)d[wn][wm][i + j*4];
                    }
                }
            }
        }
