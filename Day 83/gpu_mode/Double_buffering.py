    ...
    for (size_t innerRowOffsetB = 0; innerRowOffsetB < BK; innerRowOffsetB += strideB) {
      if ((tileOffset + innerRowOffsetB + innerRowB) < K &&
        ...
        *(__hip_fp8x4_e4m3_fnuz *)&Bs[current_buffer][innerRowOffsetB + innerRowB][innerColB * VECTOR_SIZE] = tmp;
        ...
        if (threadIdx.x == 0) {
          Ws[current_buffer][BN] = B_scale[((tileOffset + innerRowOffsetB + innerRowB) / BLOCK_DIM) * cdiv(M, BLOCK_DIM) + ((colOffsetB + innerColB * VECTOR_SIZE) / BLOCK_DIM)];
        }
      } else {
    ...
    if (tileOffset + BK < K) {
      for (size_t innerRowOffsetA = 0; innerRowOffsetA < BK; innerRowOffsetA += strideA) {
        if ((tileOffset + BK + innerRowOffsetA + innerRowA) < K &&
            (colOffsetA + innerColA * VECTOR_SIZE) < N) {
          __hip_fp8x4_e4m3_fnuz tmp = *(__hip_fp8x4_e4m3_fnuz *)&A[(tileOffset + BK + innerRowOffsetA + innerRowA) * N + (colOffsetA + innerColA * VECTOR_SIZE)];
          *(__hip_fp8x4_e4m3_fnuz *)&As[next_buffer][innerRowOffsetA + innerRowA][innerColA * VECTOR_SIZE] = tmp;

          if (threadIdx.x < (BN / VECTOR_SIZE)) {
            float4 tmp = *(float4 *)&A_scale[((tileOffset + BK + innerRowOffsetA + innerRowA) / BLOCK_DIM) * N + (colOffsetA + innerColA * VECTOR_SIZE)];
      ...
      for (size_t innerRowOffsetB = 0; innerRowOffsetB < BK; innerRowOffsetB += strideB) {
        if ((tileOffset + BK + innerRowOffsetB + innerRowB) < K &&
            (colOffsetB + innerColB * VECTOR_SIZE) < M) {
          __hip_fp8x4_e4m3_fnuz tmp = *(__hip_fp8x4_e4m3_fnuz *)&B[(tileOffset + BK + innerRowOffsetB + innerRowB) * M + (colOffsetB + innerColB * VECTOR_SIZE)];
          *(__hip_fp8x4_e4m3_fnuz *)&Bs[next_buffer][innerRowOffsetB + innerRowB][innerColB * VECTOR_SIZE] = tmp;

          if (threadIdx.x == 0) {
            Ws[next_buffer][BN] = B_scale[((tileOffset + BK + innerRowOffsetB + innerRowB) / BLOCK_DIM) * cdiv(M, BLOCK_DIM) + ((colOffsetB + innerColB * VECTOR_SIZE) / BLOCK_DIM)];
          }
        } else {
