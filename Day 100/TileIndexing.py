...
fp8_mm_kernel(const __hip_fp8_e4m3_fnuz *A, const __hip_fp8_e4m3_fnuz *B,
  ...
  for (uint16_t tileIdx = blockIdx.x; tileIdx < numTiles; tileIdx += SM_COUNT) {
    ...
    for (uint16_t innerRowOffsetA = 0; innerRowOffsetA < BK;
      ...
      if ((innerRowOffsetA + innerRowA) < K && (colOffsetA + innerColA) < N &&
        ...
        uint8_t y = *reinterpret_cast<const uint8_t *>(
            &A[(innerRowOffsetA + innerRowA + 1) * N +
               (colOffsetA + innerColA)]);
        uint8_t z = *reinterpret_cast<const uint8_t *>(
            &A[(innerRowOffsetA + innerRowA + 2) * N +
               (colOffsetA + innerColA)]);
        uint8_t w = *reinterpret_cast<const uint8_t *>(
            &A[(innerRowOffsetA + innerRowA + 3) * N +
               (colOffsetA + innerColA)]);

        unsigned int pack =
            (w & 0xff) << 24 | (z & 0xff) << 16 | (y & 0xff) << 8 | (x & 0xff);
    ...
    uint32_t A_temp1[2], A_temp2[2], A_temp3[2], A_temp4[2];
    uint32_t B_temp1[2], B_temp2[2], B_temp3[2], B_temp4[2];
    float4 A_scale_temp;
    float B_scale_temp;
      ...
      if (tileOffset < K) {
        ...
        for (uint16_t innerRowOffsetA = 0; innerRowOffsetA < BK;
          ...
          if ((tileOffset + innerRowOffsetA + innerRowA) < K &&
              ...
              (innerRowOffsetA + innerRowA) < BK) {
            A_temp1[innerRowOffsetA / strideA] =
                *reinterpret_cast<const uint8_t *>(
                    &A[(tileOffset + innerRowOffsetA + innerRowA) * N +
                       (colOffsetA + innerColA)]);
            A_temp2[innerRowOffsetA / strideA] =
                *reinterpret_cast<const uint8_t *>(
                    &A[(tileOffset + innerRowOffsetA + innerRowA + 1) * N +
                       (colOffsetA + innerColA)]);
            A_temp3[innerRowOffsetA / strideA] =
                *reinterpret_cast<const uint8_t *>(
                    &A[(tileOffset + innerRowOffsetA + innerRowA + 2) * N +
                       (colOffsetA + innerColA)]);
            A_temp4[innerRowOffsetA / strideA] =
                *reinterpret_cast<const uint8_t *>(
                    &A[(tileOffset + innerRowOffsetA + innerRowA + 3) * N +
                       (colOffsetA + innerColA)]);
          } else if ((innerRowOffsetA + innerRowA) < BK) {
            A_temp1[innerRowOffsetA / strideA] = 0;
            A_temp2[innerRowOffsetA / strideA] = 0;
            A_temp3[innerRowOffsetA / strideA] = 0;
            A_temp4[innerRowOffsetA / strideA] = 0;
          }
        }
        ...
        for (uint16_t innerRowOffsetB = 0; innerRowOffsetB < BK;
          ...
          if ((tileOffset + innerRowOffsetB + innerRowB) < K &&
              ...
              (innerRowOffsetB + innerRowB) < BK) {
            B_temp1[innerRowOffsetB / strideB] =
                *reinterpret_cast<const uint8_t *>(
                    &B[(tileOffset + innerRowOffsetB + innerRowB) * M +
                       (colOffsetB + innerColB)]);
            B_temp2[innerRowOffsetB / strideB] =
                *reinterpret_cast<const uint8_t *>(
                    &B[(tileOffset + innerRowOffsetB + innerRowB + 1) * M +
                       (colOffsetB + innerColB)]);
            B_temp3[innerRowOffsetB / strideB] =
                *reinterpret_cast<const uint8_t *>(
                    &B[(tileOffset + innerRowOffsetB + innerRowB + 2) * M +
                       (colOffsetB + innerColB)]);
            B_temp4[innerRowOffsetB / strideB] =
                *reinterpret_cast<const uint8_t *>(
                    &B[(tileOffset + innerRowOffsetB + innerRowB + 3) * M +
                       (colOffsetB + innerColB)]);
          } else if ((innerRowOffsetB + innerRowB) < BK) {
            B_temp1[innerRowOffsetB / strideB] = 0;
            B_temp2[innerRowOffsetB / strideB] = 0;
            B_temp3[innerRowOffsetB / strideB] = 0;
            B_temp4[innerRowOffsetB / strideB] = 0;
          }
        }
      ...
      if (tileOffset < K) {
        ...
        for (uint16_t innerRowOffsetA = 0; innerRowOffsetA < BK;
          ...
          if ((innerRowOffsetA + innerRowA) < BK) {
            unsigned int pack =
                (A_temp4[innerRowOffsetA / strideA] & 0xff) << 24 |
                (A_temp3[innerRowOffsetA / strideA] & 0xff) << 16 |
                (A_temp2[innerRowOffsetA / strideA] & 0xff) << 8 |
                (A_temp1[innerRowOffsetA / strideA] & 0xff);
        ...
        for (uint16_t innerRowOffsetB = 0; innerRowOffsetB < BK;
          ...
          if ((innerRowOffsetB + innerRowB) < BK) {
            unsigned int pack =
                (B_temp4[innerRowOffsetB / strideB] & 0xff) << 24 |
                (B_temp3[innerRowOffsetB / strideB] & 0xff) << 16 |
                (B_temp2[innerRowOffsetB / strideB] & 0xff) << 8 |
                (B_temp1[innerRowOffsetB / strideB] & 0xff);
