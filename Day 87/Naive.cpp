...
__global__ void fp8_mm_kernel(const __hip_fp8_e4m3_fnuz *A,
  ...
  // 2 buffers for double buffering
  __shared__ __hip_fp8_e4m3_fnuz As[2][BK][BN], Bs[2][BK][BM];
  ...
  auto load_tile = [&](int buffer, size_t tileOffset) {
    for (size_t innerRowOffsetA = 0; innerRowOffsetA < BK; innerRowOffsetA += strideA) {
      if ((tileOffset + innerRowOffsetA + innerRowA) < K &&
        ...
        *reinterpret_cast<float *>(&As[buffer][innerRowOffsetA + innerRowA][innerColA * VECTOR_SIZE]) =
            *reinterpret_cast<const float *>(&A[(tileOffset + innerRowOffsetA + innerRowA) * N +
                                                (colOffsetA + innerColA * VECTOR_SIZE)]);
      } else if ((innerRowOffsetA + innerRowA) < BK) {
        *reinterpret_cast<float *>(&As[buffer][innerRowOffsetA + innerRowA][innerColA * VECTOR_SIZE]) = 0.0f;
      }
    }
    ...
    for (size_t innerRowOffsetB = 0; innerRowOffsetB < BK; innerRowOffsetB += strideB) {
      if ((tileOffset + innerRowOffsetB + innerRowB) < K &&
        ...
        *reinterpret_cast<float *>(&Bs[buffer][innerRowOffsetB + innerRowB][innerColB * VECTOR_SIZE]) =
            *reinterpret_cast<const float *>(&B[(tileOffset + innerRowOffsetB + innerRowB) * M +
                                                (colOffsetB + innerColB * VECTOR_SIZE)]);
      } else if ((innerRowOffsetB + innerRowB) < BK &&
                 (innerColB * VECTOR_SIZE) < BM) {
        *reinterpret_cast<float *>(&Bs[buffer][innerRowOffsetB + innerRowB][innerColB * VECTOR_SIZE]) = 0.0f;
      }
    }

    if (threadIdx.x == numThreads - 1) { // One thread loads B_scale
  ...
  // Load first tile
  load_tile(curr_buffer, 0);
  __syncthreads();

  float b_scale = Ws[BN];

  // Main loop over tiles
  for (size_t tileOffset = BK; tileOffset < K; tileOffset += BK) {
    int next_buffer = curr_buffer ^ 1;

    // Start loading next tile into next buffer
    load_tile(next_buffer, tileOffset);

    // Compute on current buffer
    for (size_t BKOffset = 0; BKOffset < BK; BKOffset += 16) {
      for (size_t wn = 0; wn < WITERN; ++wn) {
        for (size_t i = 0; i < 8; ++i) {
          a[wn][i] = As[curr_buffer][BKOffset + warpY * 8 + i][warpRowOffset + wn * 32 + warpX];
        }
      }
      for (size_t wm = 0; wm < WITERM; ++wm) {
        for (size_t i = 0; i < 8; ++i) {
          b[wm][i] = Bs[curr_buffer][BKOffset + warpY * 8 + i][warpColOffset + wm * 32 + warpX];
        }
      }
      for (size_t wn = 0; wn < WITERN; ++wn) {
        for (size_t wm = 0; wm < WITERM; ++wm) {
      ...
      for (size_t j = 0; j < 4; ++j) {
        for (size_t i = 0; i < 4; ++i) {
          if ((rowOffsetC + warpRowOffset + wn * 32 + j * 8 + warpY * 4 + i) < N &&
            ...
            C[(rowOffsetC + warpRowOffset + wn * 32 + j * 8 + warpY * 4 + i) * M +
